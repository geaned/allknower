...
class VectorTextSearchIndexer(
    ...
    override fun updateIndex(documents: List<WikiDocument>) {
        for (document in documents) {
            for (content in document.contents) {
                ...
                try {
                    ...
                    doc.add(Field("embedding_serialized", Gson().toJson(content.embedding), StringField.TYPE_STORED))
                    doc.add(KnnFloatVectorField("embedding", content.embedding.toFloatArray()))
                } catch (e: Exception) {
        ...
        logger.info { "Total documents at vector text search index: $totalDocuments" }
    }
}